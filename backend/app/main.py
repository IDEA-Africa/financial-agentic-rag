...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting IDEA Africa RAG Financial Assistant API")
    ...
    if tracing_success:
        logger.info(
            "✅ Azure AI Foundry tracing enabled - traces will be visible in AI Foundry portal"
        )
    else:
        logger.warning(
            "⚠️ Azure AI Foundry tracing setup failed - continuing without tracing"
        )
...
async def track_requests(request: Request, call_next):
    ...
    observability.track_request(
        endpoint=str(request.url.path), user_id=user_id, session_id=session_id
    )
    ...
    try:
        async with observability.trace_operation(
            ...
            observability.track_response_time(
                endpoint=str(request.url.path), duration=duration, session_id=session_id
            )
        ...
        logging.getLogger(__name__).error(
            f"Request failed: {request.url.path} - {str(e)}"
        )
        raise
...
@application.get("/")
async def root():
    return {"message": "IDEA Africa Fintech  RAG Financial knowlege app"}
        ...
        return {
            ...
            "metrics": {
                "requests_last_hour": len(
                    metrics_summary.get("requests", {}).get("recent", [])
                ),
                "errors_last_hour": len(
                    metrics_summary.get("errors", {}).get("recent", [])
                ),
                "avg_response_time": metrics_summary.get("response_times", {}).get(
                    "average", 0
                ),
            },
            "timestamp": time.time(),
