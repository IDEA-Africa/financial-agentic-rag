"""
MCP Client for Fintech RAGSystem

This client communicates with the Fintech RAG MCP Server to handle
...
class MCPClient:
    ...
    def __init__(
        self, server_command: List[str], server_env: Optional[Dict[str, str]] = None
    ):
        ...
        try:
            ...
            init_request = {
                ...
                "params": {
                    ...
                    "clientInfo": {"name": "financial-rag-client", "version": "1.0.0"},
                },
            }
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        ...
        request = {"jsonrpc": "2.0", "id": str(uuid.uuid4()), "method": "tools/list"}
    ...
    async def search_documents(
        self, query: str, document_types: Optional[List[str]] = None, top_k: int = 10
    ) -> Dict[str, Any]:
        """Search financial documents using the MCP server"""
        return await self.call_tool(
            "search_financial_documents",
            {"query": query, "document_types": document_types or [], "top_k": top_k},
        )

    async def verify_sources(self, sources: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Verify source credibility using the MCP server"""
        return await self.call_tool("verify_source_credibility", {"sources": sources})
...
class HTTPMCPClient:
    ...
    def __init__(
        self, server_url: str, timeout: int = 120
    ):  # Increased to 120 seconds for financial processing
        ...
        try:
            ...
            async with self.session.get(f"{self.server_url}/mcp/info") as response:
                if response.status == 200:
                    ...
                    self.initialized = True
                    logger.info(
                        f"HTTP MCP server connected: {self.server_info.get('name', 'Unknown')}"
                    )
                else:
                    raise Exception(
                        f"Failed to connect to MCP server: HTTP {response.status}"
                    )
    ...
    async def _send_http_request(
        ...
        try:
            ...
            logger.info(
                f"📤 Sending HTTP request to {self.server_url}/mcp/rpc: {method} (timeout: {self.timeout}s)"
            )
            logger.debug(f"Request data: {request_data}")

            async with self.session.post(
                f"{self.server_url}/mcp/rpc", json=request_data
            ) as response:
                response_received_time = time.time()
                logger.info(
                    f"📥 HTTP response received after {response_received_time - request_start:.2f}s, status: {response.status}"
                )
                ...
                if response.status == 200:
                    ...
                    logger.info(
                        f"✅ HTTP request completed successfully in {total_time:.2f}s"
                    )
                    ...
                    total_time = time.time() - request_start
                    logger.error(
                        f"❌ HTTP request failed after {total_time:.2f}s: {response.status}, response: {response_text}"
                    )
                    raise Exception(
                        f"HTTP request failed: {response.status}, response: {response_text}"
                    )

        except asyncio.TimeoutError as e:
            elapsed_time = time.time() - request_start
            logger.error(
                f"⏱️ HTTP request timed out after {elapsed_time:.2f}s (timeout was {self.timeout}s)"
            )
            raise
        except aiohttp.ClientError as e:
            elapsed_time = time.time() - request_start
            logger.error(
                f"❌ HTTP client error after {elapsed_time:.2f}s communicating with MCP server: {type(e).__name__}: {e}"
            )
            raise
        except json.JSONDecodeError as e:
            elapsed_time = time.time() - request_start
            logger.error(
                f"❌ JSON decode error after {elapsed_time:.2f}s from MCP server response: {e}"
            )
            raise
        except Exception as e:
            elapsed_time = time.time() - request_start
            logger.error(
                f"❌ Unexpected error after {elapsed_time:.2f}s communicating with HTTP MCP server: {type(e).__name__}: {e}"
            )
            raise
    ...
    async def answer_financial_question(
        ...
        try:
            ...
            logger.info(
                f"HTTPMCPClient: Got result with keys: {list(result.keys()) if isinstance(result, dict) else type(result)}"
            )
            return result
        except Exception as e:
            logger.error(
                f"HTTPMCPClient: Error in answer_financial_question: {type(e).__name__}: {e}"
            )
            raise
...
class MCPService:
    ...
    async def initialize_http_client(
        self, name: str, server_url: str, timeout: int = 120
    ):
    ...
    async def process_question_with_mcp(
        ...
        try:
            logger.info(
                f"MCPService: Processing question with MCP: {question[:100]}..."
            )
            ...
            logger.info(
                f"MCPService: Got MCP result with keys: {list(result.keys()) if isinstance(result, dict) else type(result)}"
            )
            ...
            logger.error(
                f"MCPService: Error processing question with MCP: {type(e).__name__}: {e}"
            )
    ...
    def _format_citations(self, sources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, source in enumerate(sources):
            ...
            confidence_value = source.get(
                "confidence", source.get("credibility_score", 0.0)
            )
            if isinstance(confidence_value, str):
                ...
                confidence_str = (
                    ...
                    else "medium" if confidence_value > 0.5 else "low"
                )
            ...
            citations.append(
                {
                    ...
                    "documentTitle": source.get(
                        "document_title", source.get("source", "MCP Document")
                    ),
                    "pageNumber": source.get("page_number"),
                    "sectionTitle": source.get("section_title"),
                    "confidence": confidence_str,
                    "url": source.get("url"),
                    "credibilityScore": source.get(
                        "credibility_score",
                        (
                            confidence_value
    ...
    async def initialize_financial_rag_client(self):
        ...
        mcp_server_url = getattr(settings, "MCP_SERVER_URL", None) or os.getenv(
            "MCP_SERVER_URL"
        )
        ...
        if mcp_server_url:
            ...
            env = {"PYTHONPATH": workspace_path, **os.environ}
