...
edgar.set_identity("assistant@fintech-agentic.example.com")
...
class SECDocumentService:
    ...
    def __init__(self, azure_manager: AzureServiceManager):
        ...
        self.chunker = DocumentChunker(chunk_size=settings.chunk_size)
        ...
        try:
            ...
            try:
                ...
                if search_results and len(search_results) > 0:
                    ...
                    if ticker:
                        logger.info(
                            f"Found ticker via find_company: {query} -> {ticker}"
                        )
            ...
            if query_lower in name_to_ticker:
                logger.info(
                    f"Found fallback mapping: {query} -> {name_to_ticker[query_lower]}"
                )
                company = Company(name_to_ticker[query_lower])
                return await self._get_company_info(
                    company, name_to_ticker[query_lower]
                )
            ...
            try:
                ...
                if search_results and len(search_results) > 0:
                    ...
                    best_match = search_results[
                        0
                    ]  # Get first/best result (Company object)
                    ...
                    if ticker:
                        ...
                        logger.warning(
                            f"Found company but no ticker: {best_match.name}"
                        )
    ...
    async def _get_company_info(self, company, ticker: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recent_filings = company.get_filings(
                form=["10-K", "10-Q", "8-K", "10-Q/A", "10-K/A"]
            ).head(50)
            ...
            for filing in recent_filings:
                ...
                if (
                    filings_summary[form_type]["latest_date"] is None
                    or filing.filing_date > filings_summary[form_type]["latest_date"]
                ):
                    filings_summary[form_type]["latest_date"] = filing.filing_date

            # Convert sets to sorted lists for JSON serialization
            for form_type in filings_summary:
                filings_summary[form_type]["years"] = sorted(
                    list(filings_summary[form_type]["years"]), reverse=True
                )
                if filings_summary[form_type]["latest_date"]:
                    filings_summary[form_type]["latest_date"] = filings_summary[
                        form_type
            ...
            logger.info(
                f"Found company: {company.name} with {len(recent_filings)} recent filings"
            )
            return [company_info]
    ...
    async def get_company_filings(
        self, ticker: str, form_types: List[str] = None, limit: int = 20
    ...
    async def retrieve_and_process_document(
        ...
        try:
            ...
            document_exists = await self.azure_manager.check_document_exists(
                accession_number
            )
            if document_exists:
                logger.info(
                    f"Document {accession_number} already exists in index, skipping processing"
                )
            ...
            metadata = {
                ...
                "period_end_date": (
                    ...
                    if hasattr(filing, "period_end_date") and filing.period_end_date
                    else None
                ),
                "accession_number": accession_number,
                ...
                "file_size": getattr(filing, "size", None),
                "document_url": (
                    getattr(filing.document, "url", "")
                    if hasattr(filing, "document") and filing.document
                    else ""
                ),
            }
            ...
            await report_progress(
                45.0, f"Downloaded document ({len(content):,} characters)"
            )
            ...
            await report_progress(
                70.0, f"Generated {len(chunks)} chunks, starting embeddings"
            )
            ...
            search_documents = await self._prepare_search_documents(
                chunks, token_tracker, tracking_id
            )
            if search_documents:
                await report_progress(
                    92.0, f"Indexing {len(search_documents)} documents"
                )
            ...
            return {
                ...
                "tokens_used": (
                    getattr(chunks[0], "total_tokens_used", 0) if chunks else 0
                ),
            }
    ...
    async def _process_content_with_md2chunks(
        ...
        try:
            ...
            try:
                ...
                await report_progress(
                    65.0, f"Created {len(chunks_data)} intelligent chunks"
                )
            except Exception as e:
                logger.warning(
                    f"md2chunks failed: {e}, falling back to simple chunking"
                )
                chunks_data = self._fallback_chunking(markdown_content)
                await report_progress(
                    65.0, f"Created {len(chunks_data)} fallback chunks"
                )
            ...
            for i, chunk_data in enumerate(chunks_data):
                ...
                if i % 10 == 0 and progress_callback:
                    ...
                    await progress_callback(
                        "processing",
                        percent,
                        f"Processing chunk {i+1}/{len(chunks_data)}",
                    )
    ...
    def _extract_section_info(self, chunk_content: str) -> Dict[str, str]:
        ...
        item_regex = re.compile(
            r"(>Item(\s|&#160;|&nbsp;)(1A|1B|1C|1D|2|3|4|5|6|7A|7|8|9A|9B|9C|9|10|11|12|13|14|15|16)\.?)|(ITEM\s+(1A|1B|1C|1D|2|3|4|5|6|7A|7|8|9A|9B|9C|9|10|11|12|13|14|15|16))",
            re.IGNORECASE,
        )
        ...
        if matches:
            ...
            if item_num:
                ...
                logger.debug(
                    f"Detected Item {item_num} -> {section_type} from: {item_text[:50]}..."
                )
    ...
    async def _generate_embeddings_for_chunks(
        ...
        try:
                ...
                try:
                    ...
                    if progress_callback:
                        ...
                        await progress_callback(
                            "processing",
                            percent,
                            f"Generating embedding {i+1}/{len(chunks)}",
                        )
            ...
            if token_tracker and embedding_tracking_id:
                ...
                logger.info(
                    f"🔄 FINALIZING EMBEDDING TRACKING - Success: {successful_embeddings}/{len(chunks)}"
                )
                ...
                logger.info(
                    f"✅ EMBEDDING TRACKING FINALIZED - ID: {embedding_tracking_id}"
                )

            if progress_callback:
                await progress_callback(
                    "processing", 85.0, f"Completed embeddings for {len(chunks)} chunks"
                )
            ...
            if token_tracker and embedding_tracking_id:
                logger.error(
                    f"❌ FINALIZING EMBEDDING TRACKING WITH ERROR - ID: {embedding_tracking_id}"
                )
                await token_tracker.finalize_tracking(
                    tracking_id=embedding_tracking_id,
                    success=False,
            ...
            if progress_callback:
                await progress_callback(
                    "processing", 85.0, f"Embedding generation failed: {str(e)[:50]}..."
                )
            raise

    async def _prepare_search_documents(
        self, chunks: List[DocumentChunk], token_tracker=None, tracking_id=None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for chunk in chunks:
            ...
            try:
                ...
                logger.debug(
                    f"Assessed credibility score: {credibility_score:.3f} for chunk {chunk.chunk_id}"
                )

            except Exception as e:
                logger.warning(
                    f"Failed to assess credibility for chunk {chunk.chunk_id}: {e}"
                )
        ...
        if token_tracker and credibility_tracking_id:
            logger.info(
                f"🔄 FINALIZING CREDIBILITY TRACKING - Assessed: {len(search_documents)} chunks"
            )
            await token_tracker.finalize_tracking(
            ...
            logger.info(
                f"✅ CREDIBILITY TRACKING FINALIZED - ID: {credibility_tracking_id}"
            )
    ...
    async def get_specific_filings(
        ...
        try:
            logger.info(
                f"Getting specific filings for {ticker}, forms: {form_types}, years: {years}"
            )
